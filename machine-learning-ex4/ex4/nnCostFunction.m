...
a = a .+ b;
a = [a(:)];
e = Theta1(:, [2 : size(Theta1, 2)]);
f = Theta2(:, [2 : size(Theta2, 2)]);
e = e.^2;
f = f.^2;
d = sum(e(:)) + sum(f(:));
d = lambda*d/2;
...
Theta2_grad = (s_3'*a2) /m;
Theta2_grad = Theta2_grad + [zeros(size(Theta2,1) , 1) (lambda/m)*Theta2(: , [2:end])];
Theta1_grad = (s_2'*a1) / m;
Theta1_grad = Theta1_grad + [zeros(size(Theta2,1) , 1) (lambda/m)*Theta1(: , [2:end])];
